...
void Parser::handle_option(const std::string& arg)
{
    ...
    if (option == "i" || option == "init" || option == "initial" || option == "initial_state") {
        ...
        try {
            uint32_t seed_value = std::stoul(value);
            Parser::seed = value;
        } catch (...) {
...
/**
 * @brief GETTER - Interface for getting the user-given seed specified on the command line.
 *
 * @return Returns a std::string that can be casted to a uint32_t, or an empty string to represent that the
 *  user did not give a seed (or they did but it couldn't be casted to a uint32_t).
 * @throws std::logic_error When parse() hasn't been called first.
 */
std::string Parser::get_seed()
{
    if (!Parser::parse_called) throw std::logic_error("Parser::parse() must be called first");
